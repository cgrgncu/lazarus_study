...
+ 1.6 去修改「CreateMesh_AsyncProcess」的「Event」頁面下「OnReadData」為如下程式碼。
```pascal
...
```
+ 1.7 去修改「CreateMesh_AsyncProcess」的「Event」頁面下「OnTerminate」為如下程式碼。
```pascal
...
```
+ **1.8 去修改「CreateMeshRun_ToolButton」的「Event」頁面下「OnClick」為如下程式碼。**
```
...
begin
  ...
  try
    ...
    CreateMeshModelName_Edit.Enabled:=False;
    CreateMeshColorbarMinMax_Edit.Enabled:=False;
    CreateMeshStudyAreaMeshLayerSettings_Edit.Enabled:=False;
    CreateMeshPaddingMeshLeftSettings_Edit.Enabled:=False;
    CreateMeshPaddingMeshRightSettings_Edit.Enabled:=False;
    CreateMeshPaddingMeshBottomSettings_Edit.Enabled:=False;
    CreateMeshSurfaceNodeSettings_Memo.Enabled:=False;
    CreateMeshModifyMeshResistivitySettings_Memo.Enabled:=False;
    //--------------------------------------------------------------------------
  finally
...
```
+ **1.9 原始碼最前面部分先宣告修改為如下程式碼。**
```
implementation
uses
  RegExpr, LazUTF8, Windows, ShellApi;
//--------------------------------------------------------------------------
//宣告全域變數 add by HsiupoYeh
var
  version_str: AnsiString;
...
{$R *.lfm} 
```
+ **1.10 去修改「CreateMeshOpenOutputFolder_ToolButton」的「Event」頁面下「OnClick」為如下程式碼。**
```pascal
