...
{$R *.lfm} 
```
+ 4.2 拖拉一個「LazSerial>TLazSerial」到「Form1」中。預設名稱會是「LazSerial1」。
+ 4.3 選「CheckExternalDevices_Button」，去編輯「OnClick」
```pascal
...
begin
  ...
  if (PSU_Edit.Font.Color = clGreen) then
  begin
    CheckExternalDevices_Button.Enabled := False;
...
{$R *.lfm} 
```
+ 5.2 選「ScheduleRun_Button」，去編輯「OnClick」
```pascal
